...
EXEC @r = spInsertbranch 'Dhaka', 'Lalmatia, Mohammadpur', @id OUTPUT
IF @r = 0
	SELECT @id
EXEC @r = spInsertbranch 'Jassor', 'Salna, Jassor', @id OUTPUT
IF @r = 0
	SELECT @id
EXEC @r = spInsertbranch 'Chittagong', 'Sadar, Chittagong', @id OUTPUT
IF @r = 0
	SELECT @id
EXEC @r = spInsertbranch 'PotuaKhali', 'Sadar, Brahmanbaria', @id OUTPUT
IF @r = 0
	SELECT @id
EXEC @r = spInsertbranch 'Chandpur', 'Sadar, Chandpur', @id OUTPUT
IF @r = 0
	SELECT @id
EXEC @r = spInsertbranch 'Joypurhat', 'Sadar, Joypurhat', @id OUTPUT
IF @r = 0
	SELECT @id
GO
--UPDATE DATA TO THE BRANCHES TABLE USING PROCEDURE--
DECLARE @r INT
EXEC @r=spUpdatebranch 10012, 'Chittagong', 'Mirersorai, Chittagong'
iF @r <> 0
	SELECT 'Update ok'
EXEC @r=spUpdatebranch 10013, 'Joypurhat', 'Sadar, Joypurhat'
iF @r <> 0
	SELECT 'Update ok'
EXEC @r=spUpdatebranch 10014, 'Brahmanbaria', 'Sadar, Brahmanbaria'
iF @r <> 0
	SELECT 'Update ok'
GO
...
FROM gurdians g
INNER JOIN childrens c ON g.gurdianId = c.gurdianId
RIGHT OUTER JOIN branches b ON c.branchId = b.branchId
...
----------------------------------------------------------
SELECT b.branchName, 
COUNT(c.childrenId) OVER (ORDER BY b.branchid) 'children count',
ROW_NUMBER() OVER (ORDER BY b.branchid) 'rnum',
RANK() OVER (ORDER BY b.branchid) 'rank',
DENSE_RANK() OVER (ORDER BY b.branchid) 'drank',
NTILE(2) OVER (ORDER BY b.branchid) 'tile(3)'
FROM gurdians g
INNER JOIN childrens c ON g.gurdianId = c.gurdianId 
INNER JOIN branches b ON c.branchId = b.branchId 
INNER JOIN employees e ON b.branchId = e.branchId
